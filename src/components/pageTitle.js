...
export default function PageTitle({tagType, title}){
    ...
    return(
        <>
        <Box className="text-center" sx={{ display: 'flex', justifyContent: 'center', padding: '20px', gap: '10px', marginTop: '20px'}}>
        {tagType && (
        <Chip color={chipColor} size='lg'>
            <Typography level='h1' sx={{color: backgroundColor}}>{tagType}</Typography>
        </Chip> 
          )}
